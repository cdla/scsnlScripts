...
%%	seed_roi_file = ReadList(seed_roi_file_list);
%	target_roi_file = ReadList(target_roi_file_list;
%	num_seed_roi = length(seed_roi_name);
%	num_target_roi = length(target_roi_file);
   ...
   num_target_roi  = length(target_roi_file);   
    ...
    roi_stats_matrix = zeros(num_roi_file,num_target_roi,length(contrastNames));
...
%          clear gPPI_matrix ww temp;
    ...
    for nCon = 1: length(contrastNames)
         ...
         fprintf(outfileID,fmt,gppi_matrix);
         fclose(outfileID);
         catch
...
disp('======================================================================');
     ...
     switch copy_type
          ...
          case '2' % only matrices
             ...
             fprintf('>>> copying  %s \n >>> to %s \n',[temp_file,'gPPI_matrix_con*.mat'], subject_gPPI_stats_dir);
                 for nCon = 1: length(contrastNames)
              cp_status= copyfile(fullfill(temp_file,['gPPI_matrix_con',sprintf('%d4',nCon) ,'.txt']),subject_gPPI_stats_dir,'f');
              cp_status= copyfile(fullfill(temp_file,['gPPI_matrix_con',sprintf('%d4',nCon) ,'.mat']),subject_gPPI_stats_dir,'f');
                    if cp_status ~= 1
                     error('file copying fails');
                    end
                 end
