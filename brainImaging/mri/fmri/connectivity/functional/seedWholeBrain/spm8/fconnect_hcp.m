...
sprintf('adding SPM based seed whole brain scripts path: %s\n', spmfcscript_path);
...
for irun = 1:numrun
    ...
    % Yuan added
    unix(sprintf('gunzip -fq %s', 'brainmask_fs.2.nii.gz'));
    unix(sprintf('cp -af %s %s', 'brainmask_fs.2.nii', temp_dir));
    ...
    for roicnt = 1:numroi
        ...
        %-STEP 6 --------------------------------------------------------------
        disp('Running FC .....................................................');
        stats_fmri_fconnect_noscaling_HCP(temp_dir, data_type, numtrunc, newpipeline, TR_value);
        cd(currentdir)
    end
