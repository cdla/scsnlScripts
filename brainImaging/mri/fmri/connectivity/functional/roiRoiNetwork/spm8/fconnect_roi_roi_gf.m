...
if flag_name_fashion
    stats_folder = sprintf('roi_roi_network_gs%dderi%d_wmcsf%dderi%d_mov%dderi%dbp%d_dt%d', flag_gs,deriord_gs,flag_wm_csf,deriord_wm_csf,flag_mov,deriord_mov,flag_mov_bpfilter,detrend_option);
else
    stats_folder = 'roi_roi_network';
end
...
for irun = 1:numrun
    ...
    %-Truncate seed and nuisance timeseries, demean and detrend (is spm_detrend function include demean and detrend?)
    ...
    roi_res = zeros(size(all_roi_ts_dt));
    for iroi = 1:numroi
        [b,bint,r,rint,stats] = regress(all_roi_ts_dt(:,iroi), CovMtx);
        roi_res(:,iroi) = r(:);
    end
    ...
    stats_dir = fullfile(subject_dir, stats_folder);
    ...
    save(outputf,'r2r_corr','r2r_corr_z', 'r2r_cov','r2r_pcorr','r2r_pcorr_z',...
        'subject', 'roi_res', 'all_roi_ts_dt', 'CovMtx');
