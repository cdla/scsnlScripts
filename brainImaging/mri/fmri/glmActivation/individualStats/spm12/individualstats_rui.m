...
function individualstats_neginf(SubjectI, ConfigFile)


%     spmpreprocscript_path   = '/oak/stanford/groups/menon/scsnlscripts/brainImaging/mri/fmri/preprocessing/spm12/';
%     spmanalysis_path = '/oak/stanford/groups/menon/scsnlscripts/brainImaging/mri/fmri/glmActivation/individualStats/spm12';
...
%     repaired_stats      = strtrim(paralist.repaired_stats);
    ...
    whole_brain_mask   = strtrim(paralist.whole_brain_mask);
...
%        diary off; 
    ...
    %-------  Transform task_design .m into .mat ----
...
%           diary off; 
      ...
      fprintf('>>> movement == 1  |current location is %s\n',pwd);
      save task_design.mat sess_name names onsets durations rest_exists reg_file reg_names reg_vec
    else
      fprintf('>>> movement == 0  |current location is %s\n',pwd);
  ...
  %numrun
  individualfmri(pipeline, numrun, contrastmat,foname,TR,whole_brain_mask,cvi);
...
function individualfmri (pipeline,numsess,contrastmat,foname,TR,whole_brain_mask,cvi)
...
% --:---------------------------------------------------------------------------
    ...
    matlabbatch{1}.spm.stats.fmri_spec.mask{1} =[whole_brain_mask,',1'];
    matlabbatch{1}.spm.stats.fmri_spec.bases.hrf.derivs = [0,0];   %[1,0]
    matlabbatch{1}.spm.stats.fmri_spec.sess.hpf = 800;
